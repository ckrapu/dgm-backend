...
from tensorflow.keras.layers import AveragePooling2D, UpSampling2D, Activation, LayerNormalization, Lambda 
...
def resnet_decoder(latent_dim, output_shape, filters=128, n_dense=2, dense_size=128, n_upsample=2, blocks_per_resample=2,
    ...
    x = Conv2D(kernel_size=3,filters=c,padding='same',
...
def conv_encoder(output_dim,image_shape,n_downsample=2,n_conv_per_downsample=2,
    ...
    for i in range(n_downsample):
        ...
        for j in range(n_conv_per_downsample):
            model.add(Conv2D(n_conv_units,filter_size,strides=1,padding='same',activation=activation,use_bias=use_bias))
            model.add(normalizer())
